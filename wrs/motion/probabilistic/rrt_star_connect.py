...
import wrs2
import wrs2.motion.probabilistic.rrt_star as rrtst
...
class RRTStarConnect(rrtst.RRTStar):
    ...
    @wrs2.motion.probabilistic.rrt.RRT.keep_states_decorator
    def plan(self,
             start_conf,
             goal_conf,
             obstacle_list=[],
        ...
        if self._is_goal_reached(conf=start_conf, goal_conf=goal_conf, threshold=ext_dist):
            mot_data = wrs2.motion.probabilistic.rrt.motu.MotionData(self.robot)
...
if __name__ == '__main__':
    ...
    from wrs2 import robot_sim as xyb
